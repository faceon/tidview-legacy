...
- User input (wallet) is stored in sync storage only after passing `cfg.WALLET_REGEX`; saving/refreshing fire `chrome.runtime.sendMessage({ type: "refresh" })` and optimistically toggle `isBusy`/`positionsLoading` flags.
...
- Sync keys: `wallet`, `positionsValue`, `cashValue`, `valuesUpdatedAt`, `valuesError`. Session keys: `positions`, `positionsUpdatedAt`, `positionsError`. The UI assumes these exact names and types when reconciling state.
...
- No automated tests exist; manual verification is done by saving a known 0x wallet and checking badge/UI updates.

## Conventions & Tips

- Always validate wallet strings with `cfg.WALLET_REGEX` before persisting or firing network requests; invalid wallets should surface as `"—"` in the UI and badge.
