...
function RenderPosition({ position, onOpenMarket }) {
  const avgPriceText =
    position.avgPrice != null
      ? `@ ${formatNumber(position.avgPrice, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}`
      : "";
  const curPriceText =
    position.curPrice != null
      ? `→ ${formatNumber(position.curPrice, { minimumFractionDigits: 2, maximumFractionDigits: 3 })}`
      : "";
  const sizeText =
    ...
    <li
      ...
      onKeyDown={(event) => {
        if (event.key === "Enter" || event.key === " ") {
          ...
          onOpenMarket(position.slug, position.eventSlug);
        }
      }}
    >
      {position.icon ? (
      ...
      <div className="text-right flex flex-col gap-1 text-xs">
        ...
        <div
          className={`position-stat-pnl ${trendClass(position.cashPnl)} text-[13px] font-semibold`}
        >
    ...
    <div>
      ...
      <div className="flex items-start justify-between gap-4 mb-3">
        ...
        <div className="flex flex-col text-right">
          ...
          <span
            className={`text-[13px] font-semibold ${trendClass(totalCashPnl)}`}
          >
            {formatSignedCurrency(totalCashPnl)}{" "}
            {totalPercent != null ? (
              <span className="text-xs">({formatPercent(totalPercent)})</span>
            ) : null}
          </span>
        </div>
      </div>
